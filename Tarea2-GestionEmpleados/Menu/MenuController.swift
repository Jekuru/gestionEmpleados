...
class MenuController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // Cargar imagen
        let imageUrl = returnUserInfo!["profileImgUrl"] as! String
        let url = URL(string: imageUrl)
        let data = try? Data(contentsOf: url!) //make sure your image in this url does exist, otherwise unwrap in a if let check / try-catch
        avatar.image = UIImage(data: data!)
        // Cargar rol
        var role = returnUserInfo!["job"] as! String
        role = role.prefix(1).capitalized + role.dropFirst()
        roleText.text = "Role: " + role
        // Cargar email
        let email = returnUserInfo!["email"] as! String
        emailText.text = "Email: " + email
        // Cargar salario
        let salary = returnUserInfo!["salary"] as! Int
        salaryText.text = "Salary: " + String(salary)
        // Cargar biografia
        let biography = returnUserInfo!["biography"] as! String
